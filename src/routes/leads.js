...
router.get('/stats', requireAdmin, async (req, res) => {
  const db = getDB();
  const total    = (await db.prepare('SELECT COUNT(*) as c FROM leads').get()).c;
  const today    = (await db.prepare("SELECT COUNT(*) as c FROM leads WHERE created_at >= CURRENT_DATE").get()).c;
  const hot      = (await db.prepare('SELECT COUNT(*) as c FROM leads WHERE score >= 70').get()).c;
  const byType   = await db.prepare('SELECT apartment_type, COUNT(*) as c FROM leads GROUP BY apartment_type').all();
  const byBudget = await db.prepare('SELECT budget, COUNT(*) as c FROM leads GROUP BY budget').all();
  const byTimeline = await db.prepare('SELECT timeline, COUNT(*) as c FROM leads GROUP BY timeline').all();
...
router.get('/', requireAdmin, async (req, res) => {
  ...
  if (search) {
    ...
    conditions.push(`(name ILIKE $${searchIdx} OR phone ILIKE $${searchIdx + 1} OR email ILIKE $${searchIdx + 2})`);
...
router.patch('/:id', requireAdmin, async (req, res) => {
  ...
  const db = getDB();
  await db.prepare('UPDATE leads SET status=COALESCE($1,status), notes=COALESCE($2,notes), updated_at=CURRENT_TIMESTAMP WHERE id=$3')
